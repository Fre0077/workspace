...
/*   Updated: 2025/04/10 17:06:49 by alborghi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	zero_case(t_data *data, t_viktor *tm, t_viktor dir, int witch, t_viktor *nose)
{
	...
	*nose = (t_viktor){dir.x, dir.y, (dist[2] == dist[0])};
	return (dist[2]);
}
...
double	calculate_dist(t_data *data, double angle, double ra, t_viktor *nose)
{
	...
	if (isnan(dir.y))
		dir.y = 1;
	if (r(angle) == 0)
		return (zero_case(data, &tm, dir, ((int)angle) / 90, nose));
	first_step(d, &tm, data->player, dir, nose);
	while (!hit(data, tm))
	{
		...
		if (d[0] < d[1])
			d[2] = d[0];
		*nose = (t_viktor){dir.x, dir.y, (d[2] == d[0])};
	}
	return (d[2]);
}
